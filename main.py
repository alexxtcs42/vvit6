...
class Calculator(QWidget):
    def __init__(self):
        ...
        self.hbox_fourth.addWidget(self.b_00)
        self.b_comma = QPushButton(".", self)
        self.hbox_fourth.addWidget(self.b_comma)
    ...
    def _result(self):
        if self.input.text() == 'Ошибка':
            ...
            if self.op == "+":
                self.input.setText(str(self.num_1 + self.num_2).rstrip('0').rstrip('.'))
            elif self.op == "-":
                self.input.setText(str(self.num_1 - self.num_2).rstrip('0').rstrip('.'))
            elif self.op == "*":
                self.input.setText(str(self.num_1 * self.num_2).rstrip('0').rstrip('.'))
            elif self.op == "/" and self.num_2 != 0.0:
                self.input.setText(str(self.num_1 / self.num_2).rstrip('0').rstrip('.'))
            else:
